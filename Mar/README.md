...
3.6 Rules for if-elif-else block
...
The basic syntax for defining a function in Mar is as follows:
	There are two types of functions: ``Basic Function`` and ``Smart Return Functions``
...
In this case, the output_parameters section defines the expected output of the function. 
...
**Note: The current version of Mar import is many having problems in the following areas: (we 
are working on them)
- Currently use can:
